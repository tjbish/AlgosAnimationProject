// Creates a randomize button.
// Adapted from the gale_shapley control setup.
...
function ensureControls() {
	...
	function updateVisibility() {
		button.style.display =
			algorithmSelect.value === "dinic" ? "inline-block" : "none";
	}
	...
	*createGenerator() {
		...
		while (true) {
			...
			yield {
				...
				totalFlow: maxFlow,
				flowMessage:
					"BFS built the level graph. No new flow added in this step.",
				nodes,
				edges,
			};
			...
			if (levels["t"] === null) {
				yield {
					...
					totalFlow: maxFlow,
					flowMessage:
						"No residual path reaches the sink, so the current total flow is maximum.",
					nodes,
					edges,
				};
				break;
